...
export default function ToDoList() {
    ...
    const changeStatus =  (key) =>{
        ...
        setToDoList(
            toDoList.map((todo) =>
                todo.id === key ? { ...todo, status: todo.status=== 'open' ?  'closed' : 'open'} : todo
            )
        );
    };
    ...
    return (
        <div>
            ...
            { toDoList? toDoList.map((value, key) => {
                if(filter !== 'all' && value.status !== filter) return ;

                return <div key={key}>
                    <div className={value.status==='open' ? 'todoItemDivColor' : 'todoItemDoneDivColor' }
                            key={key}
                            onClick={(e) => {
                        ...
                        <ToggleButton
                            ...
                            onLabel="Tamamla" offLabel="Tekrar Aç"
                            onIcon="pi pi-check"
                            offIcon="pi pi-times"
                        />
                </div>
            ...
            <TodoFooter totalTasks={toDoList.length}
